select *
from CONCESSIONNAIRE c;

select fv.ID_MODELE, fv.NOMBRE_DE_VENTE
from FAIT_VENTE fv;
#
update CONCESSIONNAIRE c
set NOM='Croix-rousse'
where NOM = 'Croix Rousse';

update FAIT_VENTE fv
set NOMBRE_DE_VENTE=2500,
    ID_TEMPS=(select fv.ID_TEMPS from TEMPS t where t.ANNEE = "2001")
where fv.ID_CONCESSIONNAIRE = (select c.ID_CONCESSIONNAIRE from CONCESSIONNAIRE c where c.NOM = 'Bron')
  and fv.ID_MODELE = (select m.ID_MODELE from MODEL m where m.DESIGNATION = 'Espace')
  and fv.ID_TEMPS = (select t.ID_TEMPS from TEMPS t where t.ANNEE = 2002);

SELECT c.NOM,
       m.DESIGNATION           as 'Modèle',
       t.ANNEE                 AS 'Année',
       SUM(fv.NOMBRE_DE_VENTE) AS 'nombre de ventes',
       COUNT(*)                AS nb_lignes
FROM FAIT_VENTE fv
         JOIN CONCESSIONNAIRE c ON c.ID_CONCESSIONNAIRE = fv.ID_CONCESSIONNAIRE
         JOIN MODEL m ON m.ID_MODELE = fv.ID_MODELE
         JOIN TEMPS t ON t.ID_TEMPS = fv.ID_TEMPS
GROUP BY c.NOM, m.DESIGNATION, t.ANNEE, fv.NOMBRE_DE_VENTE;

# exo 1
SELECT c.NOM,
       fv.NOMBRE_DE_VENTE
FROM FAIT_VENTE fv
         JOIN CONCESSIONNAIRE c ON c.ID_CONCESSIONNAIRE = fv.ID_CONCESSIONNAIRE;

# exo 2
SELECT c.NOM,
       t.ANNEE,
       fv.NOMBRE_DE_VENTE
FROM FAIT_VENTE fv
         JOIN CONCESSIONNAIRE c ON c.ID_CONCESSIONNAIRE = fv.ID_CONCESSIONNAIRE
         JOIN TEMPS t ON t.ID_TEMPS = fv.ID_TEMPS
ORDER BY t.ANNEE;

# exo 3
SELECT DISTINCT c.NOM,
                t.ANNEE,
                fv.NOMBRE_DE_VENTE
FROM FAIT_VENTE fv
         JOIN CONCESSIONNAIRE c ON c.ID_CONCESSIONNAIRE = fv.ID_CONCESSIONNAIRE
         JOIN TEMPS t ON t.ID_TEMPS = fv.ID_TEMPS
ORDER BY t.ANNEE;

# exo 4
SELECT c.NOM,
       t.ANNEE,
       fv.NOMBRE_DE_VENTE
FROM FAIT_VENTE fv
         JOIN CONCESSIONNAIRE c ON c.ID_CONCESSIONNAIRE = fv.ID_CONCESSIONNAIRE
         JOIN TEMPS t ON t.ID_TEMPS = fv.ID_TEMPS
WHERE fv.NOMBRE_DE_VENTE >= 1500
  AND fv.NOMBRE_DE_VENTE <= 5000
ORDER BY t.ANNEE;

# exo 4 bis
SELECT c.NOM,
       t.ANNEE,
       fv.NOMBRE_DE_VENTE
FROM FAIT_VENTE fv
         JOIN CONCESSIONNAIRE c ON c.ID_CONCESSIONNAIRE = fv.ID_CONCESSIONNAIRE
         JOIN TEMPS t ON t.ID_TEMPS = fv.ID_TEMPS
WHERE t.ANNEE >= 2000
  AND t.ANNEE <= 2005
  AND fv.NOMBRE_DE_VENTE <= 3000
ORDER BY t.ANNEE;

# exo 5
SELECT c.NOM,
       SUM(fv.NOMBRE_DE_VENTE)
FROM FAIT_VENTE fv
         JOIN CONCESSIONNAIRE c ON c.ID_CONCESSIONNAIRE = fv.ID_CONCESSIONNAIRE
GROUP BY c.NOM;

# exo 6
SELECT t.ANNEE,
       SUM(fv.NOMBRE_DE_VENTE)
FROM FAIT_VENTE fv
         JOIN TEMPS t ON t.ID_TEMPS = fv.ID_TEMPS
GROUP BY t.ANNEE;

# exo 7
SELECT c.NOM,
       SUM(fv.NOMBRE_DE_VENTE)
FROM FAIT_VENTE fv
         JOIN CONCESSIONNAIRE c ON c.ID_CONCESSIONNAIRE = fv.ID_CONCESSIONNAIRE
GROUP BY c.NOM
HAVING SUM(fv.NOMBRE_DE_VENTE) > 2000;

# exo 8
SELECT c.NOM,
       SUM(fv.NOMBRE_DE_VENTE)
FROM FAIT_VENTE fv
         JOIN CONCESSIONNAIRE c ON c.ID_CONCESSIONNAIRE = fv.ID_CONCESSIONNAIRE
WHERE c.NOM LIKE '%O%'
GROUP BY c.NOM
HAVING SUM(fv.NOMBRE_DE_VENTE) > 2000;

# exo 9
SELECT c.NOM,
       AVG(fv.NOMBRE_DE_VENTE)
FROM FAIT_VENTE fv
         JOIN CONCESSIONNAIRE c ON c.ID_CONCESSIONNAIRE = fv.ID_CONCESSIONNAIRE
GROUP BY c.NOM
HAVING AVG(fv.NOMBRE_DE_VENTE) > 1000;

# exo 10
SELECT t.ANNEE,
       SUM(fv.NOMBRE_DE_VENTE) as 'total_ventes'
FROM FAIT_VENTE fv
         JOIN TEMPS t ON t.ID_TEMPS = fv.ID_TEMPS
GROUP BY t.ANNEE
ORDER BY total_ventes DESC
LIMIT 1;

-- Partie trigger;

# exo 1
create trigger insert_modele_audit
    BEFORE INSERT ON MODEL
    FOR EACH ROW
    INSERT INTO AUDIT_TABLE (LOG_DATE, ACTION, TABLE_NAME, MODELE_ID)
    values (NOW(), 'INSERT', 'MODEL', NEW.ID_MODELE);

create trigger insert_fait_vente_audit
    BEFORE INSERT ON FAIT_VENTE
    FOR EACH ROW
    INSERT INTO AUDIT_TABLE (LOG_DATE, ACTION, TABLE_NAME, MODELE_ID, TEMPS_ID, CON_ID)
    values (NOW(), 'INSERT', 'FAIT_VENTE', NEW.ID_MODELE, NEW.ID_TEMPS, NEW.ID_CONCESSIONNAIRE);

INSERT INTO `MODEL` (`DESIGNATION`) VALUES
    ('Test Insert');

INSERT INTO `FAIT_VENTE` (`ID_CONCESSIONNAIRE`, `ID_MODELE`, `ID_TEMPS`, `NOMBRE_DE_VENTE`) VALUES(
                                                                                                      (select ID_CONCESSIONNAIRE FROM  CONCESSIONNAIRE WHERE NOM='Bron'),
                                                                                                      (select ID_MODELE FROM  MODEL WHERE DESIGNATION='Test Insert'),
                                                                                                      (select ID_TEMPS FROM  TEMPS WHERE ANNEE=2001), 55500);